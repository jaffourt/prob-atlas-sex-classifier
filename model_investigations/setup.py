...
def main():
    ...
    param_grid = {
        ...
        'max_depth': [20, 80, 120, None],
        'min_samples_split' : [2, 5, 10, 50],
        'min_samples_leaf' : [1, 2, 4, 10],
        'max_features': ['auto', 'sqrt'],
        'n_estimators': [int(x) for x in np.linspace(start = 350, stop = 650, num = 10)]
    }
    labels_train=np.asarray(labels_train)
    ...
    for iter in range(iterations):
        ...
        clf.fit(np.reshape(all_images_train, (all_images_train.shape[0], -1)),labels_train)
